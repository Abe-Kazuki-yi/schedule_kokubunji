error id: file://<WORKSPACE>/src/main/scala/controller/AuthController.scala:controller/AuthController#EmailLoginRequest#email.
file://<WORKSPACE>/src/main/scala/controller/AuthController.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -play/api/mvc/data/email.
	 -play/api/mvc/data/email#
	 -play/api/mvc/data/email().
	 -play/api/libs/json/data/email.
	 -play/api/libs/json/data/email#
	 -play/api/libs/json/data/email().
	 -play/api/libs/ws/data/email.
	 -play/api/libs/ws/data/email#
	 -play/api/libs/ws/data/email().
	 -javax/inject/data/email.
	 -javax/inject/data/email#
	 -javax/inject/data/email().
	 -struct/RoleEnum.data.email.
	 -struct/RoleEnum.data.email#
	 -struct/RoleEnum.data.email().
	 -struct/AccountTypeEnum.data.email.
	 -struct/AccountTypeEnum.data.email#
	 -struct/AccountTypeEnum.data.email().
	 -data/email.
	 -data/email#
	 -data/email().
	 -scala/Predef.data.email.
	 -scala/Predef.data.email#
	 -scala/Predef.data.email().
offset: 2308
...
extends AbstractController(cc) {
     ...
     */
    case class PasswordLoginRequest(usename: String, password: String)
    ...
    /**
     * @brief usernameでのログインを処理するコントローラ
     * @param　EmailLoginRequestと同じjson { username, password }
        ...
        data => {
            ...
            tokenRequest.flatMap { tokenResp =>
                ...
                userInfoReq.flatMap { userResp =>
                    ...
                    authService.findGoogleUser(googleId).map {
                        case Some(user) =>
