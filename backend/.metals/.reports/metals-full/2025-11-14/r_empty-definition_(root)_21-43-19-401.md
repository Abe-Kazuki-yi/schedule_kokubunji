error id: file://<WORKSPACE>/src/main/scala/repository/UserRepository.scala:Email
file://<WORKSPACE>/src/main/scala/repository/UserRepository.scala
empty definition using pc, found symbol in pc: 
semanticdb not found

found definition using fallback; symbol Email
offset: 1113
...
class UserRepository @Inject()(db: Database)(implicit ec: ExecutionContext) {
    ...
    def createUserByEmail(username: String, password: String, role: Role): Future[Int] = {
        val user : User = User(0L, Some(username), Some(password), None, Pa@@ssword, role)
        db.run(users += user)
    }
