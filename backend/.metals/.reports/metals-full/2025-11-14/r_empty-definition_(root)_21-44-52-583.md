error id: file://<WORKSPACE>/src/main/scala/repository/UserRepository.scala:mail
file://<WORKSPACE>/src/main/scala/repository/UserRepository.scala
empty definition using pc, found symbol in pc: 
semanticdb not found

found definition using fallback; symbol mail
offset: 1752
...
class UserRepository @Inject()(db: Database)(implicit ec: ExecutionContext) {
    ...
    def createUserByGoogle(googleId: String, role: Role): Future[Int] = {
        val user : User = User(0L, None, None, Some(googleId), Google, role)
        db.run(users += user)
    }
     ...
     */
    def findByMail(username: String): Future[Option[User]] = {
        db.run(users.filter(_.us@@ername === mail).result.headOption)
    }
