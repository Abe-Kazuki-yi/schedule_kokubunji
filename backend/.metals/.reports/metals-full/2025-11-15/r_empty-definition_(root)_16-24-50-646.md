error id: file://<WORKSPACE>/src/main/scala/controller/AuthController.scala:utility/JwtUtil#createToken().
file://<WORKSPACE>/src/main/scala/controller/AuthController.scala
empty definition using pc, found symbol in pc: 
found definition using semanticdb; symbol utility/JwtUtil#createToken().
...
extends AbstractController(cc) {
     ...
     */
    case class GoogleAuthRequest(code: String, role: Role)
    ...
    /**
     * @brief googlユーザの新規登録をするメソッドを使ったログインを処理するコントローラ
        ...
        data => {
            ...
            tokenRequest.flatMap { tokenResp =>
                ...
                userInfoReq.flatMap { userResp =>
                    ...
                    // GoogleIdでユーザの登録
                    userRepository.createUserByGoogle(googleId, role).map {
                        case Some(user) =>
                            val token = jwtUtil.createTo@@ken(user.id, user.role)isoiso
                            Ok(Json.obj(
                                "message" -> "Login success",
                                "token" -> token,
                                "role" -> user.role
